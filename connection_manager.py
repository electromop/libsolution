...
class ConnectionManager:
    def __init__(self):
        ...
        self.connections: Dict[int, Dict[str, dict]] = defaultdict(dict)  # journal_id -> user_id -> {ws, name, email, color, avatar}
    ...
    async def connect(self, websocket: WebSocket, journal_id: int, user_email: str):
        ...
        # Генерируем "аватар" (эмодзи) из фиксированного списка
        avatars = ["🐶","🐱","🦊","🐼","🐨","🐯","🦁","🐵","🐸","🦄","🐹","🐰","🐙","🐝","🦋"]
        avatar = avatars[len(self.connections[journal_id]) % len(avatars)]
    ...
    async def broadcast_user_list(self, journal_id: int):
        users = [{"id": uid, "name": u["name"], "email": u.get("email"), "color": u["color"], "avatar": u.get("avatar")}
                 for uid, u in self.connections[journal_id].items()]
